...
int main(void)
{
    ...
    fp = fopen("opt_v1.txt", "r");
    if (!fp) {
        printf("ERROR opening input file opt_v1.txt\n");
        exit(0);
    }
    ...
    fp = fopen("opt_v2.txt", "r");
    if (!fp) {
        printf("ERROR opening input file opt_v2.txt\n");
        exit(0);
    }
    double opt_sum2_a = 0.0, opt_sum2_f = 0.0, opt2_a, opt2_f;
    for (i = 0; i < 100; i++) {
        if (feof(fp)) {
            printf("ERROR: You need 100 datum instead of %d\n", i);
            printf("run 'make run' longer to get enough information\n\n");
